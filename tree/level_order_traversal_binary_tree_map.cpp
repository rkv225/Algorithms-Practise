...
void print_level_order_helper(node* root, int level)
{
	if (root == NULL) return;
	level_map[level].push_back(root); // add current node to list
	print_level_order_helper(root->left, level + 1); // traverse left node
	print_level_order_helper(root->right, level + 1); // traverse right node
}

void print_level_order(node* root)
{
	print_level_order_helper(root, 1);
